...
export default function WelcomeGate() {
  ...
  const { triggerEntered } = useWelcome();

  useEffect(() => {
    // Only show on the home route
    if (pathname !== "/") return;
    // Use sessionStorage so it resets on every new browser session / tab
    const alreadySeen = sessionStorage.getItem("sw_welcomed") === "1";
    if (!alreadySeen && !hasEnteredSession) {
      setShow(true);
    }
  }, [pathname]);
  ...
  const handleEnter = () => {
    ...
    sessionStorage.setItem("sw_welcomed", "1");
    triggerEntered();
